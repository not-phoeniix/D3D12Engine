...
namespace RayTracing {
    ...
    namespace {
        ...
        UINT64 tlasInstanceDataSizeInBytes[Graphics::NUM_BACK_BUFFERS] {};
...
void RayTracing::CreateEntityDataBuffer(std::vector<std::shared_ptr<GameEntity>> scene) {
    ...
    EntityDataStructuredBuffer = Graphics::CreateBuffer(
        sizeof(RayTracingEntityData) * data.size(),
    ...
    if (EntityDataUAV_CPU.ptr == 0) {
        Graphics::ReserveDescriptorHeapSlot(&EntityDataUAV_CPU, &EntityDataUAV_GPU);
    }

    D3D12_UNORDERED_ACCESS_VIEW_DESC uav_desc = {};
    uav_desc.ViewDimension = D3D12_UAV_DIMENSION_BUFFER;
    uav_desc.Buffer.NumElements = static_cast<UINT>(data.size());
    uav_desc.Buffer.StructureByteStride = sizeof(RayTracingEntityData);
...
MeshRayTracingData RayTracing::CreateBottomLevelAccelerationStructureForMesh(Mesh* mesh) {
    ...
    geometryDesc.Triangles.VertexBuffer.StartAddress = mesh->get_vb()->GetGPUVirtualAddress();
    geometryDesc.Triangles.VertexBuffer.StrideInBytes = mesh->get_vb_view().StrideInBytes;
    geometryDesc.Triangles.VertexCount = static_cast<UINT>(mesh->get_vertex_count());
    geometryDesc.Triangles.VertexFormat = DXGI_FORMAT_R32G32B32_FLOAT;
    geometryDesc.Triangles.IndexBuffer = mesh->get_ib()->GetGPUVirtualAddress();
    geometryDesc.Triangles.IndexFormat = mesh->get_ib_view().Format;
    geometryDesc.Triangles.IndexCount = static_cast<UINT>(mesh->get_index_count());
    ...
    indexSRVDesc.Buffer.NumElements = (UINT)mesh->get_index_count();
    indexSRVDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
    DXRDevice->CreateShaderResourceView(mesh->get_ib().Get(), &indexSRVDesc, ib_cpu);
    ...
    vertexSRVDesc.Buffer.NumElements = (UINT)((mesh->get_vertex_count() * sizeof(Vertex)) / sizeof(float)); // How many floats total?
    vertexSRVDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
    DXRDevice->CreateShaderResourceView(mesh->get_vb().Get(), &vertexSRVDesc, vb_cpu);
...
void RayTracing::CreateTopLevelAccelerationStructureForScene(std::vector<std::shared_ptr<GameEntity>> scene) {
    ...
    std::vector<D3D12_RAYTRACING_INSTANCE_DESC> instance_descs(scene.size());
    for (size_t i = 0; i < scene.size(); i++) {
        // Grab the entity's transform and transpose to column major
        DirectX::XMFLOAT4X4 transform = scene[i]->get_transform().GetWorldMatrix();
        XMStoreFloat4x4(&transform, XMMatrixTranspose(XMLoadFloat4x4(&transform)));

        // Describe the BLAS instance(s) that make up the TLAS
        instance_descs[i].InstanceID = 0;
        instance_descs[i].InstanceContributionToHitGroupIndex = 0;
        instance_descs[i].InstanceMask = 0xFF;
        memcpy(&instance_descs[i].Transform, &transform, sizeof(float) * 3 * 4); // Copy first [3][4] elements
        instance_descs[i].AccelerationStructure = scene[i]->get_mesh()->get_raytracing_data().BLAS->GetGPUVirtualAddress();
    ...
    unsigned int frameIndex = Graphics::get_swap_chain_index();
    ...
    accelStructInputs.NumDescs = static_cast<UINT>(instance_descs.size());
    accelStructInputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE;
...
void RayTracing::Raytrace(std::shared_ptr<Camera> camera, Microsoft::WRL::ComPtr<ID3D12Resource> currentBackBuffer) {
    ...
    DirectX::XMStoreFloat4x4(&sceneData.InverseViewProj, XMMatrixInverse(0, vp));

    D3D12_GPU_DESCRIPTOR_HANDLE cbuffer = Graphics::CBHeapFillNext(&sceneData, sizeof(RaytracingSceneData));
