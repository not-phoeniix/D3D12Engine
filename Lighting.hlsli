...
#define LIGHT_MAX_SPECULAR_EXPONENT 256.0f
...
float3 PhongSpecular(float3 direction, float3 toCam, float intensity, float3 color, float3 normal, float roughness, float3 worldPos) {
    float specularExp = (1.0f - roughness) * LIGHT_MAX_SPECULAR_EXPONENT;
    float3 refl = reflect(direction, normal);
    float3 specularTerm =
        pow(max(dot(refl, toCam), 0), specularExp) *
        intensity *
        color;
    specularTerm = specularExp > 0.01f ? specularTerm : 0.0f;

    return specularTerm;
}

float3 LambertDiffuse(float3 direction, float intensity, float3 color, float3 normal) {
    // compute floats first for slight efficiency!
...
float3 LightDirectionalPBR(Light light, PSInput input, float3 camera_world_pos, float roughness, float3 specular_color, float metalness, float3 surface_color) {
	float3 to_cam = normalize(camera_world_pos - input.world_pos);

    float diffuse = DiffusePBR(input.normal, -light.direction);
    float3 f_out;
