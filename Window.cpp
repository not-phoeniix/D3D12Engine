...
HRESULT Window::Create(
    ...
    windowHandle = CreateWindow(
        ...
        0
    ); // No other windows in our application
...
void Window::UpdateStats(float totalTime) {
    ...
    output.precision(6);
    output << windowTitle << "    Width: " << windowWidth << "    Height: " << windowHeight << "    FPS: " << fpsFrameCounter << "    Frame Time: " << mspf << "ms" << "    Graphics: " << Graphics::get_api_name();
...
void Window::CreateConsoleWindow(int bufferLines, int bufferColumns, int windowLines, int windowColumns) {
    ...
    SetConsoleMode(GetStdHandle(STD_OUTPUT_HANDLE), currentMode | ENABLE_PROCESSED_OUTPUT | ENABLE_VIRTUAL_TERMINAL_PROCESSING);
