...
class Mesh {
   ...
   public:
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource> get_vb() const { return vertex_buffer; }
    Microsoft::WRL::ComPtr<ID3D12Resource> get_ib() const { return index_buffer; }
    uint32_t get_vertex_count() const { return num_vertices; }
    uint32_t get_index_count() const { return num_indices; }
    const MeshRayTracingData& get_raytracing_data() const { return rt_data; }
